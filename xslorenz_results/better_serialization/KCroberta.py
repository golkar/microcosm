...
class KCRobertaForMaskedLM(RobertaForMaskedLM):
    ...
    def __init__(self, config):
        ...
        self.roberta = KCRobertaModel(config, add_pooling_layer=False)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # clipping the numbers from below by 10^-6
            MSE_target = numbers.sign() * (numbers.abs().clamp(min=1e-6).log10() + 6)
            masked_numbers_loss = loss_fct_numbers(
                prediction_numbers, MSE_target.type_as(prediction_numbers)
            )
        ...
        return MaskedLMOutputWithNumbers(
            ...
            numbers=prediction_numbers.sign() * (10 ** (prediction_numbers.abs() - 6)),
...
class KCRobertaModel(RobertaModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = KCRobertaEmbeddings(config)
...
class KCRobertaEmbeddings(RobertaEmbeddings):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            masked_numbers_log = masked_numbers.sign() * (
                masked_numbers.abs().clamp(min=1e-6).log10() + 6
            )
            inputs_embeds = inputs_embeds * masked_numbers_log.unsqueeze(-1).type_as(
                inputs_embeds
            )
